...
from app.backend.schemas.user import UserCreate, UserLogin, UserPublic, TokenResponse
...
def login(user_credentials: UserLogin, db: Session = Depends(get_db)):
    ...
    return TokenResponse(
        access_token=token,
        token_type="Bearer", 
        user=UserPublic(first_name=user.first_name, last_name=user.last_name, email=user.email))
