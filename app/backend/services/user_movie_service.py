...
def update_user_movie_status(tmdb_id: int, user_id: int, database: Session, status: str) -> Tuple[bool, str]:
    
    print(tmdb_id, user_id, status)
    try :
        existing_user_movie = database.query(UserMovie).where(
            UserMovie.user_id==user_id, 
            UserMovie.tmdb_id==tmdb_id).first()
        ...
        if status=="none":  
            ...
            if existing_user_movie:
                ...
                new_user_movie = UserMovie(user_id=user_id, tmdb_id=tmdb_id, status=status)
...
def get_user_movies_by_status(user_id: int, database: Session, language: str, status: str) -> MovieListResponse:
